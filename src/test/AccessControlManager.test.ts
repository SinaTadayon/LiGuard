...
describe('AccessControlManager', function() {
    ...
    describe('Subject (AccessControlMnanager Implementation)', function() {
        ...
        it('Should deploy AccessControlManager subject success', async () => {
            ...
            expect(await accessControlManagerSubject.isActivated()).to.be.equal(false);
            expect(await accessControlManagerSubject.isUpgradable()).to.be.equal(false);
            expect(await accessControlManagerSubject.getInitializedCount()).to.be.equal(0);
        })

        it('Should initialize raise exception', async() => {            
            //when and then
            await expect(accessControlManagerSubject.connect(admin)
                .initialize('AccessControlManagement', 'v0.0.1', 'LIVELY_GENERAL_REALM', accessControlManagerSubject.address))
                .to.be.revertedWith('Illegal Contract Call');                
        })

        it('Should setActivity raise exception', async() => {
            //when and then
            await expect(accessControlManagerSubject.connect(admin).setActivity(true))
                .to.be.revertedWith("Illegal Contract Call")
        })

        it('Should setUpgradability raise exception', async() => {
            //when and then
            await expect(accessControlManagerSubject.connect(admin).setUpgradability(true))
                .to.be.revertedWith("Illegal Contract Call")
        })
        ...
        it('Should setAdmin raise exception', async() => {
            ...
            //when and then
            await expect(accessControlManagerSubject.connect(admin).setAdmin(address))
                .to.be.revertedWith("Illegal Contract Call")
        })
        ...
        it('Should upgradeTo raise exception', async() => {
            ...
            //when and then
            await expect(accessControlManagerSubject.connect(admin).upgradeTo(accessControlManagerSubject.address, typedArray1, false))
                .to.be.revertedWith("Illegal Contract Call")
        })
        ...
        it('Should return slot storage of subject', async() => {
            //when and then
            expect(await accessControlManagerSubject.proxiableUUID())
                .to.be.hexEqual('0x360894a13ba1a3210667c828492db98dca3e2076cc3735a920a3ca505d382bbc')
        })
    })
    ...
    describe('Proxy (UUPS Proxy Implementation)', function() {
        ...
        it('Should proxy deploy success without initialize (typechain)', async() => {
            ...
            // when 
            const acmProxy = await proxyFactory.deploy(accessControlSubject.address, new Int8Array(0));
            ...
            // and
            expect(await accessControlProxy.isActivated(), "Invalid Activated").to.be.false;
            expect(await accessControlProxy.isUpgradable(), "Invalid Upgradablability").to.be.false;
            expect(await accessControlProxy.getInitializedCount(), "Invalid InitailizedCount").to.be.equal(0);
        })
        ...
        it('Should proxy deploy success with initialize', async() => {
            ...
            let iface = new ethers.utils.Interface(accessControlArtifact.abi);
            const data = iface.encodeFunctionData('initialize', ["AccessControlManager", "v0.0.1", "LIVELY_GENERAL_REALM", ethers.constants.AddressZero])
    
            // when
            const proxyFactory = await ethers.getContractFactory('Proxy');
            const acProxy = await proxyFactory.connect(admin).deploy(accessControlSubject.address, data);
            ...
            // and            
            expect(await accessControlManager.isActivated(), "Invalid Activated").to.be.true;
            expect(await accessControlManager.isUpgradable(), "Invalid Upgradablability").to.be.false;
            expect(await accessControlManager.getInitializedCount(), "Invalid InitailizedCount").to.be.equal(1);
            expect(await accessControlManager.isInitializing(), "Invalid Initializing").to.be.false;
            expect(await accessControlManager.getAdmin(), "Invalid Admin").to.be.hexEqual(acProxy.address);
            expect(await accessControlManager.contractName(), "Invalid Name").to.be.equal("AccessControlManager");
            expect(await accessControlManager.contractVersion(), "Invalid Version").to.be.equal("v0.0.1");
            ...
            expect(await accessControlManager.getAccessControl(), "Invalid Access Control").to.be.hexEqual(acProxy.address);
            expect(await accessControlManager.contractContext(), "Invalid Context").to.be.hexEqual(ethers.utils.keccak256(acProxy.address));
        ...
        it('Should proxy deploy and initialize success (typechain)', async() => {           
            ...
            // when 
            const acProxy = await proxyFactory.deploy(accessControlManagerSubject.address, new Int8Array(0));
            accessControlManagerProxy = accessControlManagerSubject.attach(acProxy.address);
            await accessControlManagerProxy.connect(admin).initialize("AccessControlManager", "v0.0.1", "LIVELY_GENERAL_REALM", ethers.constants.AddressZero)

            // then
            expect(await accessControlManagerProxy.isActivated(), "Invalid Activated").to.be.true;
            expect(await accessControlManagerProxy.isUpgradable(), "Invalid Upgradablability").to.be.false;
            expect(await accessControlManagerProxy.getInitializedCount(), "Invalid InitailizedCount").to.be.equal(1);
            expect(await accessControlManagerProxy.isInitializing(), "Invalid Initializing").to.be.false;
            expect(await accessControlManagerProxy.getAdmin(), "Invalid Admin").to.be.hexEqual(acProxy.address);
            expect(await accessControlManagerProxy.contractName(), "Invalid Name").to.be.equal("AccessControlManager");
            expect(await accessControlManagerProxy.contractVersion(), "Invalid Version").to.be.equal("v0.0.1");
            ...
            expect(await accessControlManagerProxy.getAccessControl(), "Invalid Access Control").to.be.hexEqual(acProxy.address);
            expect(await accessControlManagerProxy.contractContext(), "Invalid Context").to.be.hexEqual(ethers.utils.keccak256(acProxy.address));
        ...
        it('Should proxiableUUID raised exception', async() => {
            //when and then
            await expect(accessControlManagerProxy.proxiableUUID())
        ...
        it('Should setUpgradability enabled', async() => {
            ...
            // when and then
            await expect(accessControlManagerProxy.connect(user).setUpgradability(true))
                .to.emit(accessControlManagerProxy, 'UpgradabilityChanged')
                .withArgs(userAddress, accessControlManagerProxy.address, ethers.utils.keccak256(ethers.utils.toUtf8Bytes("LIVELY_GENERAL_REALM")), true);
        })
        ...
        it('Should upgradeTo success', async() => {
            ...
            await expect(accessControlManagerProxy.connect(user).upgradeTo(accessControlManagerSubject.address, typedArray1, false))
                   ...
                   .withArgs(userAddress, accessControlManagerProxy.address, accessControlManagerSubject.address)
        ...
        it('Should setActivity disabled', async() => {
            ...
            await expect(accessControlManagerProxy.connect(user).setActivity(false))
                ...
                .withArgs(userAddress, accessControlManagerProxy.address, ethers.utils.keccak256(ethers.utils.toUtf8Bytes("LIVELY_GENERAL_REALM")), false);
        })    
        ...
        it('Should setAdmin raise exception', async() => {
            ...
            //when and then
            await expect(accessControlManagerProxy.connect(user).setAdmin(address))
