...
describe("Lively Guard Tests", function () {
  ...
  describe("ACLManager Subject Tests", function () {
    ...
    it("Should setSafeModeStatus raise exception", async () => {
      // when and then
      await expect(
        aclManagerSubject.connect(systemAdmin).setSafeModeStatus(ProxySafeModeStatus.DISABLED)
    ...
    it("Should setUpgradeState raise exception", async () => {
      // when and then
      await expect(
        aclManagerSubject.connect(systemAdmin).setUpdatabilityStatus(ProxyUpdatabilityStatus.ENABLED)
      ).to.be.revertedWith("Illegal Call");
    });
    ...
    it("Should setLocalAdmin raise exception", async () => {
      ...
      // when and then
      await expect(aclManagerSubject.connect(systemAdmin).setLocalAdmin(address)).to.be.revertedWith("Illegal Call");
    });
  ...
  describe("ACL Manager Proxy (UUPS Proxy Implementation) Tests", function () {
    ...
    it("Should enable UpgradeState of proxy by systemAdmin failed when not initialized", async () => {
      // when and then
      await expect(
        aclManagerProxy.connect(systemAdmin).setUpdatabilityStatus(ProxyUpdatabilityStatus.ENABLED)
      ).to.revertedWith("Rejected");
    });

    it("Should setLocalAdmin of proxy by systemAdmin failed when not initialized", async () => {
      // when and then
      await expect(aclManagerProxy.connect(systemAdmin).setLocalAdmin(userWallet1.address)).to.revertedWith("Rejected");
    });
  ...
  describe("ACL Modules Proxies Tests", function () {
    it("Should deploy proxy MemberManager with invalid acl failed", async () => {
      ...
      // when
      await expect(proxyFactory.deploy(memberManagerSubject.address, data)).revertedWith("Illegal Contract");
    });
    ...
    it("Should deploy proxy MemberManager with illegal acl failed", async () => {
      ...
      // when
      await expect(proxyFactory.deploy(memberManagerSubject.address, data)).revertedWith("Illegal ACL");
    });
  ...
  describe("ACL Profiles Modules Proxies Tests", function () {
    it("Should deploy proxy ProfileMemberManager with invalid acl failed", async () => {
      ...
      // when
      await expect(proxyFactory.deploy(profileMemberManagerSubject.address, data)).revertedWith("Illegal Contract");
    });
    ...
    it("Should deploy proxy ProfileMemberManager with illegal acl failed", async () => {
      ...
      // when
      await expect(proxyFactory.deploy(profileMemberManagerSubject.address, data)).revertedWith("Illegal ACL");
    });
  ...
  describe("ACLManager proxy base function Tests", function () {
    ...
    it("Should upgrade proxy by common user1 failed", async () => {
      // given
      const aclManagerSubjectFactory = new ACLManager__factory(
        <ACLManagerLibraryAddresses>linkCommonLibraryAddresses,
        systemAdmin
      );
    ...
    it("Should upgrade proxy by livelyAdmin success", async () => {
      ...
      // when and then
      await expect(aclManagerProxy.connect(systemAdmin).upgradeTo(aclManagerSubject.address, typedArray1, false))
    ...
    it("Should enable SafeMode proxy by user1 failed", async () => {
      // when and then
      await expect(aclManagerProxy.connect(user1).setSafeModeStatus(ProxySafeModeStatus.ENABLED)).to.revertedWith(
        "Forbidden"
      );
    });
    ...
    it("Should enable SafeMode proxy by livelyAdmin success", async () => {
      ...
      await expect(aclManagerProxy.connect(livelyAdmin).setSafeModeStatus(ProxySafeModeStatus.ENABLED))
        ...
        .withArgs(livelyAdminWallet.address, aclManagerProxy.address, ProxySafeModeStatus.ENABLED);
    });
    ...
    it("Should register ACL_TYPE_TEST failed (in safe mode)", async () => {
      ...
      // when
      await expect(
        typeManagerDelegateProxy.connect(livelyAdmin).typeRegister(memberSignature, typeRegisterRequests)
      ).to.revertedWith("Rejected");
    });

    it("Should setAdmin proxy failed (in safeMode)", async () => {
      // when and then
      await expect(aclManagerProxy.connect(livelyAdmin).setLocalAdmin(userWallet1.address)).to.revertedWith("Rejected");
    });
    ...
    it("Should disable SafeMode of proxy by livelyAdmin success", async () => {
      ...
      await expect(aclManagerProxy.connect(livelyAdmin).setSafeModeStatus(ProxySafeModeStatus.DISABLED))
        .to.emit(aclManagerProxy, "ProxySafeModeUpdated")
        .withArgs(livelyAdminWallet.address, aclManagerProxy.address, ProxySafeModeStatus.DISABLED);
    });

    it("Should setAccessControlManager with invalid acl failed", async () => {
      // when and then
      await expect(
        aclManagerProxy.connect(livelyAdmin).setAccessControlManager(userWallet1.address)
      ).to.be.revertedWith("Illegal Contract");
    });
  ...
  describe("Agent Tests", function () {
    it("Should disable activity of LIVELY_VERSE_LIVELY_MASTER_TYPE_ID success", async () => {
      // given
      const requests: IACLCommonsRoles.UpdateActivityRequestStruct[] = [
        {
          id: LIVELY_VERSE_LIVELY_MASTER_TYPE_ID,
      ...
      await expect(typeManagerDelegateProxy.connect(livelyAdmin).typeUpdateActivityStatus(memberSignature, requests))
        ...
        .withArgs(livelyAdminWallet.address, LIVELY_VERSE_LIVELY_MASTER_TYPE_ID, ActivityStatus.DISABLED);
    });

    it("Should enable activity of LIVELY_VERSE_LIVELY_MASTER_TYPE_ID success", async () => {
      // given
      const requests: IACLCommonsRoles.UpdateActivityRequestStruct[] = [
        {
          id: LIVELY_VERSE_LIVELY_MASTER_TYPE_ID,
      ...
      await expect(typeManagerDelegateProxy.connect(livelyAdmin).typeUpdateActivityStatus(memberSignature, requests))
        .to.emit(typeManagerDelegateProxy, "TypeActivityUpdated")
        .withArgs(livelyAdminWallet.address, LIVELY_VERSE_LIVELY_MASTER_TYPE_ID, ActivityStatus.ENABLED);
    });

    it("Should disabled activity of LIVELY_VERSE_LIVELY_MASTER_ADMIN_ID success", async () => {
      // given
      const requests: IACLCommonsRoles.UpdateActivityRequestStruct[] = [
        {
          id: LIVELY_VERSE_LIVELY_MASTER_ADMIN_ROLE_ID,
          acstat: ActivityStatus.DISABLED,
        },
      ];
      const memberSignature: IACLCommons.MemberSignatureStruct = {
      ...
      await expect(roleManagerDelegateProxy.connect(livelyAdmin).roleUpdateActivityStatus(memberSignature, requests))
        ...
        .withArgs(livelyAdminWallet.address, LIVELY_VERSE_LIVELY_MASTER_ADMIN_ROLE_ID, ActivityStatus.DISABLED);
    });

    it("Should enabled activity of LIVELY_VERSE_LIVELY_MASTER_ADMIN_ID success", async () => {
      // given
      const requests: IACLCommonsRoles.UpdateActivityRequestStruct[] = [
        {
          id: LIVELY_VERSE_LIVELY_MASTER_ADMIN_ROLE_ID,
    ...
    it("Should register test type with expired memberSignature failed", async () => {
      ...
      // when
      await expect(
        typeManagerDelegateProxy.connect(systemAdmin).typeRegister(memberSignature, typeRegisterRequests)
    ...
    it("Should register test type with memberSignature success", async () => {
      ...
      await expect(typeManagerDelegateProxy.connect(systemAdmin).typeRegister(memberSignature, typeRegisterRequests))
        ...
        .withArgs(livelyAdminWallet.address, typeTestId, realmContextId, LIVELY_VERSE_ACL_ADMIN_ROLE_ID);
    ...
    it("Should register ACL_TYPE_TEST success", async () => {
      ...
      await expect(typeManagerDelegateProxy.connect(livelyAdmin).typeRegister(memberSignature, typeRegisterRequests))
        .to.emit(typeManagerDelegateProxy, "TypeRegistered")
        .withArgs(livelyAdminWallet.address, aclTypeTestId, realmContextId, LIVELY_VERSE_ACL_ADMIN_ROLE_ID);
      ...
      // and
      const realmContextInfo: IContextManagement.ContextInfoStruct = await contextManagerDelegateProxy.contextGetInfo(
        realmContextId
      );
      expect(realmContextInfo.adminId).to.be.equal(LIVELY_VERSE_ACL_TYPE_ID);
      expect(realmContextInfo.adminType).to.be.equal(AgentType.TYPE);
      expect(realmContextInfo.contractId).to.be.equal(realmManagerProxy.address);
      expect(realmContextInfo.realmId).to.be.equal(LIVELY_VERSE_ACL_REALM_ID);
      expect(realmContextInfo.functionLimit).to.be.equal(32);
      expect(realmContextInfo.functionCount).to.be.equal(13);
      expect(realmContextInfo.acstat).to.be.equal(ActivityStatus.ENABLED);
      expect(realmContextInfo.alstat).to.be.equal(AlterabilityStatus.UPGRADABLE);
      expect(realmContextInfo.referredByAgent).to.be.equal(2);
    });

    it("Should remove ACL_TYPE_TEST success", async () => {
      // given
      const realmContextId = ethers.utils.keccak256(realmManagerProxy.address);
      const memberSignature: IACLCommons.MemberSignatureStruct = {
        account: ethers.constants.AddressZero,
      ...
      // when
      await expect(typeManagerDelegateProxy.connect(livelyAdmin).typeRemove(memberSignature, [aclTypeTestId]))
        .to.emit(typeManagerDelegateProxy, "TypeRemoved")
        .withArgs(livelyAdminWallet.address, aclTypeTestId);

      // then
      expect(await typeManagerDelegateProxy.typeCheckId(aclTypeTestId)).to.be.false;
      ...
      // and
      expect(await typeManagerDelegateProxy.typeCheckId(aclTypeTestId)).to.be.false;
      expect(await typeManagerDelegateProxy.typeCheckName(ACL_TYPE_TEST_NAME)).to.be.false;
      expect(await typeManagerDelegateProxy.typeCheckAdmin(aclTypeTestId, livelyAdminWallet.address)).to.be.false;
    ...
    it("Should disable type alterability of ACL_TYPE_TEST success", async () => {
      ...
      // when
      await expect(
        typeManagerDelegateProxy.connect(livelyAdmin).typeUpdateAlterabilityStatus(memberSignature, requests)
      )
    ...
    it("Should update admin of ACL_TYPE_TEST when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        typeManagerDelegateProxy.connect(livelyAdmin).typeUpdateAdmin(memberSignature, updateAdminRequests)
    ...
    it("Should update roleLimit of ACL_TYPE_TEST when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        typeManagerDelegateProxy.connect(livelyAdmin).typeUpdateRoleLimit(memberSignature, requests)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should update activity of ACL_TYPE_TEST when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        typeManagerDelegateProxy.connect(livelyAdmin).typeUpdateActivityStatus(memberSignature, requests)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should update scope of ACL_TYPE_TEST when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        typeManagerDelegateProxy.connect(livelyAdmin).typeUpdateScope(memberSignature, requests)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should disable activity of ACL_TYPE_TEST by anonymous failed", async () => {
      ...
      // when
      await expect(
        typeManagerDelegateProxy.connect(user1).typeUpdateActivityStatus(memberSignature, requests)
    ...
    it("Should disable activity of ACL_TYPE_TEST by anyone failed", async () => {
      ...
      // when
      await expect(
        typeManagerDelegateProxy.connect(systemAdmin).typeUpdateActivityStatus(memberSignature, requests)
    ...
    it("Should register aclRoleTest in ACL_TYPE_TEST type with illegal scope failed", async () => {
      // given
      ...
      // when
      await expect(
        roleManagerDelegateProxy.connect(livelyAdmin).roleRegister(memberSignature, roleRegisterRequests)
    ...
    it("Should register aclRoleTest4 in ACL_TYPE_TEST type success", async () => {
      // given
      const memberContextId = ethers.utils.keccak256(memberManagerProxy.address);
      const memberRegisterFunctionId = ethers.utils.keccak256(
        ethers.utils.solidityPack(
      ...
      expect(functionInfo.acstat).to.be.equal(ActivityStatus.ENABLED);
      expect(functionInfo.alstat).to.be.equal(AlterabilityStatus.UPDATABLE);
      expect(functionInfo.referredByAgent).to.be.equal(2);
    });

    it("Should remove aclRoleTest4 success", async () => {
      // given
      const memberContextId = ethers.utils.keccak256(memberManagerProxy.address);
      const memberRegisterFunctionId = ethers.utils.keccak256(
        ethers.utils.solidityPack(
      ...
      // when
      await expect(roleManagerDelegateProxy.connect(systemAdmin).roleRemove(memberSignature, [aclRoleTestId4]))
        .to.emit(roleManagerDelegateProxy, "RoleRemoved")
        .withArgs(livelyAdminWallet.address, aclRoleTestId4);

      // then
      expect(await roleManagerDelegateProxy.roleCheckId(aclRoleTestId4)).to.be.false;
      ...
      // and
      expect(await roleManagerDelegateProxy.roleCheckId(aclRoleTestId4)).to.be.false;
      expect(await roleManagerDelegateProxy.roleCheckName(ACL_ROLE_TEST_NAME_4)).to.be.false;
      expect(await roleManagerDelegateProxy.roleCheckAdmin(aclRoleTestId4, livelyAdminWallet.address)).to.be.false;
    ...
    it("Should ACL_TYPE_TEST update admin by anyone failed", async () => {
      ...
      // when
      await expect(
        typeManagerDelegateProxy.connect(user1).typeUpdateAdmin(memberSignature, updateAdminRequests)
      ).revertedWith("ACLMemberNotFound()");
    });
    ...
    it("Should register aclRoleTest in ACL type with illegal RegisterLimit failed", async () => {
      ...
      // when
      await expect(
        roleManagerDelegateProxy.connect(systemAdmin).roleRegister(memberSignature, roleRegisterRequests)
    ...
    it("Should update systemAdmin generalLimit with anyone failed", async () => {
      ...
      // when
      await expect(
        memberManagerDelegateProxy.connect(user1).memberUpdateGeneralLimit(memberSignature, requests)
      ).revertedWith("ACLMemberNotFound()");
    });
    ...
    it("Should update systemAdmin generalLimit with Illegal TypeLimit failed", async () => {
      ...
      // when
      await expect(
        memberManagerDelegateProxy.connect(livelyAdmin).memberUpdateGeneralLimit(memberSignature, requests)
    ...
    it("Should update roleLimit of ACL_TYPE_TEST failed", async () => {
      ...
      // when
      await expect(
        typeManagerDelegateProxy.connect(systemAdmin).typeUpdateRoleLimit(memberSignature, requests)
    ...
    it("Should update admin of aclRoleTest when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        roleManagerDelegateProxy.connect(systemAdmin).roleUpdateAdmin(memberSignature, updateAdminRequests)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should update activity of aclRoleTest when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        roleManagerDelegateProxy.connect(systemAdmin).roleUpdateActivityStatus(memberSignature, requests)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should update memberLimit of aclRoleTest when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        roleManagerDelegateProxy.connect(systemAdmin).roleUpdateMemberLimit(memberSignature, memberLimitRequests)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should grant member to aclRoleTest when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        roleManagerDelegateProxy.connect(systemAdmin).roleGrantMembers(memberSignature, requests)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should revoke member to aclRoleTest when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        roleManagerDelegateProxy.connect(systemAdmin).roleRevokeMembers(memberSignature, requests)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should aclRoleTest register new member when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        memberManagerDelegateProxy.connect(systemAdmin).memberRegister(memberSignature, memberRegisterRequests)
    ...
    it("Should update alterability of aclRoleTest success", async () => {
      ...
      // when
      await expect(
        roleManagerDelegateProxy.connect(livelyAdmin).roleUpdateAlterabilityStatus(memberSignature, requests)
      )
    ...
    it("Should aclRoleTest register user1 with illegal admin failed", async () => {
      ...
      // when
      await expect(
        memberManagerDelegateProxy.connect(livelyAdmin).memberRegister(memberSignature, memberRegisterRequests)
    ...
    it("Should aclRoleTest register user1 success", async () => {
      ...
      await expect(
        ...
        .withArgs(
          ...
          aclRoleTestId,
          LIVELY_VERSE_MEMBER_MASTER_TYPE_ID,
          [32, 32, 32, 32, 32, 32, 32, 32, 1, 32, 2, 32, 32, 32, 32, 32, 32, 32]
        );
    ...
    it("Should grant systemAdmin to livelyAdmin role failed", async () => {
      ...
      // when
      await expect(
        roleManagerDelegateProxy.connect(livelyAdmin).roleGrantMembers(memberSignature, requests)
      ).revertedWith("Illegal Grant");
    });

    it("Should grant systemAdmin to aclRoleTestId role success", async () => {
      // given
      const systemAdminId = ethers.utils.keccak256(systemAdminWallet.address);
      const requests: IRoleManagement.RoleGrantMembersRequestStruct[] = [
        {
    ...
    it("Should aclRoleTest activity disabled with someone failed", async () => {
      ...
      // when
      await expect(
        roleManagerDelegateProxy.connect(user1).roleUpdateActivityStatus(memberSignature, requests)
    ...
    it("Should register user2 to aclRoleTest success", async () => {
      ...
      await expect(memberManagerDelegateProxy.connect(livelyAdmin).memberRegister(memberSignature, requests))
        ...
        .withArgs(
          ...
          aclRoleTestId,
          aclRoleTestId,
          [16, 16, 16, 16, 16, 16, 16, 16, 15, 16, 5, 16, 16, 16, 16, 16, 16, 16]
        );
    ...
    it("Should aclRoleTest register another member when meet limitation failed", async () => {
      ...
      // when
      await expect(memberManagerDelegateProxy.connect(user1).memberRegister(memberSignature, requests)).revertedWith(
        "Illegal Register"
      );
    });
    ...
    it("Should aclRoleTest update member limit by someone failed", async () => {
      ...
      // when
      await expect(
        roleManagerDelegateProxy.connect(user1).roleUpdateMemberLimit(memberSignature, requests)
      ).revertedWith("Illegal Limit");
    });
    ...
    it("Should aclRoleTest update member limit with old admin failed", async () => {
      ...
      // when
      await expect(
        roleManagerDelegateProxy.connect(livelyAdmin).roleUpdateMemberLimit(memberSignature, requests)
      ).revertedWith("AdminAccessNotPermitted()");
    });
    ...
    it("Should register user3 member to aclRoleTest failed", async () => {
      ...
      // when
      await expect(memberManagerDelegateProxy.connect(user1).memberRegister(memberSignature, requests)).to.revertedWith(
        "CallForbidden()"
      );
    });
    ...
    it("Should register user3 member to aclRoleTest success", async () => {
      ...
      await expect(memberManagerDelegateProxy.connect(user1).memberRegister(memberSignature, requests))
        ...
        .withArgs(
          ...
          aclRoleTestId,
          LIVELY_VERSE_MEMBER_MASTER_TYPE_ID,
          [8, 8, 8, 8, 8, 8, 8, 8, 2, 8, 1, 8, 8, 8, 8, 8, 8, 8]
        );
    ...
    it("Should remove user3 member success", async () => {
      // given
      const roleMemberUserId3 = ethers.utils.keccak256(userWallet3.address);
      const memberSignature: IACLCommons.MemberSignatureStruct = {
        account: ethers.constants.AddressZero,
      ...
      // when
      await expect(memberManagerDelegateProxy.connect(livelyAdmin).memberRemove(memberSignature, [roleMemberUserId3]))
        .to.emit(memberManagerDelegateProxy, "MemberRoleRevoked")
        .withArgs(livelyAdminWallet.address, roleMemberUserId3, aclRoleTestId, aclTypeTestId)
        .to.emit(memberManagerDelegateProxy, "MemberRemoved")
        .withArgs(livelyAdminWallet.address, roleMemberUserId3, true);

      // then
      expect(await memberManagerDelegateProxy.memberCheckId(roleMemberUserId3)).to.be.false;
      ...
      expect(memberInfo.limits.memberLimit).to.be.equal(0);
      expect(memberInfo.limits.memberRegisterLimit).to.be.equal(0);
      expect(memberInfo.limits.contextRegisterLimit).to.be.equal(0);
      expect(memberInfo.limits.functionRegisterLimit).to.be.equal(0);
      expect(memberInfo.limits.profileRegisterLimit).to.be.equal(0);
      expect(memberInfo.limits.contextLimit).to.be.equal(0);
      expect(memberInfo.limits.realmLimit).to.be.equal(0);
      expect(memberInfo.limits.domainLimit).to.be.equal(0);
      expect(memberInfo.limits.callLimit).to.be.equal(0);
      expect(memberInfo.limits.typeRoleLimit).to.be.equal(0);
      expect(memberInfo.limits.typeLimit).to.be.equal(0);
      expect(memberInfo.limits.roleRegisterLimit).to.be.equal(0);
      expect(memberInfo.limits.typeRegisterLimit).to.be.equal(0);
      expect(memberInfo.limits.realmRegisterLimit).to.be.equal(0);
      expect(memberInfo.limits.domainRegisterLimit).to.be.equal(0);
      expect(memberInfo.limits.policyRegisterLimit).to.be.equal(0);
      expect(memberInfo.limits.policyRoleLimit).to.be.equal(0);
      expect(memberInfo.limits.functionLimit).to.be.equal(0);
    ...
    it("Should grant members to aclAdminRole success", async () => {
      ...
      // then
      expect(await roleManagerDelegateProxy.roleHasAccount(LIVELY_VERSE_ACL_ADMIN_ROLE_ID, userWallet1.address)).to.be
        .true;
      expect(await roleManagerDelegateProxy.roleHasAccount(LIVELY_VERSE_ACL_ADMIN_ROLE_ID, userWallet2.address)).to.be
        .true;
      ...
      // and
      expect(await memberManagerDelegateProxy.memberGetTypes(userId1)).to.be.eqls([
        aclTypeTestId,
        LIVELY_VERSE_ACL_TYPE_ID,
      ]);
      expect(await memberManagerDelegateProxy.memberGetTypes(userId2)).to.be.eqls([
        aclTypeTestId,
        LIVELY_VERSE_ACL_TYPE_ID,
      ]);
      ...
      expect(await typeManagerDelegateProxy.typeHasRole(LIVELY_VERSE_ACL_TYPE_ID, LIVELY_VERSE_ACL_ADMIN_ROLE_ID)).to.be
        .true;

      // and
      expect(await typeManagerDelegateProxy.typeGetRoles(LIVELY_VERSE_ACL_TYPE_ID)).to.be.eqls([
        LIVELY_VERSE_ACL_ADMIN_ROLE_ID,
      ]);
      expect(await typeManagerDelegateProxy.typeGetRoles(aclTypeTestId)).to.be.eqls([
        aclRoleTestId4,
        aclRoleTestId,
        aclRoleTestId2,
        aclRoleTestId3,
    ...
    it("Should update member alterability of user1 failed", async () => {
      ...
      // when
      await expect(
        memberManagerDelegateProxy.connect(user1).memberUpdateAlterabilityStatus(memberSignature, requests)
    ...
    it("Should disable member alterability of user1 success", async () => {
      ...
      // when
      await expect(
        memberManagerDelegateProxy.connect(livelyAdmin).memberUpdateAlterabilityStatus(memberSignature, requests)
      )
    ...
    it("Should update admin of user1 when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        memberManagerDelegateProxy.connect(livelyAdmin).memberUpdateAdmin(memberSignature, updateAdminRequests)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should user1 activity disabled success", async () => {
      ...
      expect(memberInfo.typeCount).to.be.equal(1);
      expect(memberInfo.limits.memberLimit).to.be.equal(16);
      expect(memberInfo.limits.memberRegisterLimit).to.be.equal(14);
      expect(memberInfo.limits.contextRegisterLimit).to.be.equal(16);
      expect(memberInfo.limits.functionRegisterLimit).to.be.equal(16);
      expect(memberInfo.limits.profileRegisterLimit).to.be.equal(16);
      expect(memberInfo.limits.contextLimit).to.be.equal(16);
      expect(memberInfo.limits.realmLimit).to.be.equal(16);
      expect(memberInfo.limits.domainLimit).to.be.equal(16);
      expect(memberInfo.limits.callLimit).to.be.equal(29);
      expect(memberInfo.limits.typeRoleLimit).to.be.equal(16);
    ...
    it("Should remove ACL_TYPE_TEST failed", async () => {
      // given
      aclTypeTestId = ethers.utils.keccak256(ethers.utils.toUtf8Bytes(ACL_TYPE_TEST_NAME));
  ...
  describe("Policy Test", function () {
    it("Should register aclPolicyTest in ACL policy success", async () => {
      ...
      // and
      const policyInfo: IPolicyManagement.PolicyInfoStruct = await policyManagerDelegateProxy.policyGetInfo(
        aclPolicyTestId
      );
      ...
      expect(memberContextInfo.realmId).to.be.equal(LIVELY_VERSE_ACL_REALM_ID);
      expect(memberContextInfo.functionLimit).to.be.equal(32);
      expect(memberContextInfo.functionCount).to.be.equal(12);
      expect(memberContextInfo.acstat).to.be.equal(ActivityStatus.ENABLED);
      expect(memberContextInfo.alstat).to.be.equal(AlterabilityStatus.UPGRADABLE);
      expect(memberContextInfo.referredByAgent).to.be.equal(1);
    });

    it("Should remove aclPolicyTest success", async () => {
      // given
      const memberContextId = ethers.utils.keccak256(memberManagerProxy.address);
      aclPolicyTestId = ethers.utils.keccak256(ethers.utils.toUtf8Bytes(ACL_POLICY_TEST_NAME));
      const memberSignature: IACLCommons.MemberSignatureStruct = {
      ...
      // when
      await expect(policyManagerDelegateProxy.connect(livelyAdmin).policyRemove(memberSignature, [aclPolicyTestId]))
        .to.emit(policyManagerDelegateProxy, "PolicyRemoved")
        .withArgs(livelyAdminWallet.address, aclPolicyTestId);

      // then
      expect(await policyManagerDelegateProxy.policyCheckId(aclPolicyTestId)).to.be.false;
      ...
      // and
      expect(await policyManagerDelegateProxy.policyCheckId(aclPolicyTestId)).to.be.false;
      expect(await policyManagerDelegateProxy.policyCheckName(ACL_POLICY_TEST_NAME)).to.be.false;
      expect(await policyManagerDelegateProxy.policyCheckAdmin(aclPolicyTestId, livelyAdminWallet.address)).to.be.false;
    ...
    it("Should add aclRoleTestId4 to aclPolicyTest success", async () => {
      ...
      // and
      const policyInfo: IPolicyManagement.PolicyInfoStruct = await policyManagerDelegateProxy.policyGetInfoByRole(
        aclRoleTestId4
      );
      expect(policyInfo.name).to.be.equal(ACL_POLICY_TEST_NAME);
      expect(policyInfo.adminId).to.be.equal(LIVELY_VERSE_LIVELY_MASTER_ADMIN_ROLE_ID);
    ...
    it("Should update scope aclPolicyTest in ACL policy failed", async () => {
      ...
      // when
      await expect(
        policyManagerDelegateProxy.connect(livelyAdmin).policyUpdateScope(memberSignature, requests)
    ...
    it("Should disable alterability of aclPolicyTest success", async () => {
      ...
      // when
      await expect(
        policyManagerDelegateProxy.connect(systemAdmin).policyUpdateAlterabilityStatus(memberSignature, requests)
      )
    ...
    it("Should update admin of aclPolicyTest when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        policyManagerDelegateProxy.connect(livelyAdmin).policyUpdateAdmin(memberSignature, updateAdminRequests)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should update policyCode of aclPolicyTest when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        policyManagerDelegateProxy.connect(livelyAdmin).policyUpdateCodes(memberSignature, requests)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should update activity of aclPolicyTest when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        policyManagerDelegateProxy.connect(livelyAdmin).policyUpdateActivityStatus(memberSignature, requests)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should update roleLimit of aclPolicyTest when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        policyManagerDelegateProxy.connect(livelyAdmin).policyUpdateRoleLimit(memberSignature, requests)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should add roles to aclPolicyTest when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        policyManagerDelegateProxy.connect(livelyAdmin).policyAddRoles(memberSignature, requests)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should update alterability of aclPolicyTest success", async () => {
      ...
      // when
      await expect(
        policyManagerDelegateProxy.connect(livelyAdmin).policyUpdateAlterabilityStatus(memberSignature, requests)
      )
        .to.emit(policyManagerDelegateProxy, "PolicyAlterabilityUpdated")
    ...
    it("Should add roles to aclPolicyTest for illegal role failed", async () => {
      ...
      // when
      await expect(policyManagerDelegateProxy.connect(user1).policyAddRoles(memberSignature, requests)).revertedWith(
        "Illegal Role"
      );
    });
    ...
    it("Should add roles to aclPolicyTest for limitation failed", async () => {
      ...
      // when
      await expect(
        policyManagerDelegateProxy.connect(systemAdmin).policyAddRoles(memberSignature, requests)
      ).revertedWith("Illegal Limit");
    });
    ...
    it("Should add aclRoleTestId to aclPolicyTest success", async () => {
      ...
      expect(await policyManagerDelegateProxy.policyGetRoles(aclPolicyTestId)).to.be.eqls([
        aclRoleTestId4,
        aclRoleTestId2,
        aclRoleTestId,
      ]);
    ...
    it("Should update policyCode of aclPolicyTest to UNLOCK success", async () => {
      ...
      const isPolicyAccess = await policyManagerDelegateProxy.policyCheckAccess(
        aclPolicyTestId,
        memberRegisterFunctionId
      );
      const isRoleAccess = await policyManagerDelegateProxy.policyCheckRoleAccess(
        aclRoleTestId,
        memberRegisterFunctionId
      );
      const requests: IPolicyManagement.PolicyUpdateCodeRequestStruct[] = [
      ...
      expect(await policyManagerDelegateProxy.policyCheckRoleAccess(aclRoleTestId, memberRegisterFunctionId)).to.be
        .true;
    });

    it("Should remove aclPolicyTest failed", async () => {
      // given
      aclPolicyTestId = ethers.utils.keccak256(ethers.utils.toUtf8Bytes(ACL_POLICY_TEST_NAME));
      const memberSignature: IACLCommons.MemberSignatureStruct = {
        account: ethers.constants.AddressZero,
  ...
  describe("Scope Test", function () {
    ...
    it("Should hard remove policyRemoveFunction success", async () => {
      // given
      const policyIface = new ethers.utils.Interface(PolicyManager__factory.abi);
      const policyRemoveFunctionId = ethers.utils.keccak256(
        ethers.utils.solidityPack(
    ...
    it("Should soft remove memberRemoveFunction success", async () => {
      ...
      expect(functionInfo.adminType).to.be.equal(AgentType.TYPE);
      expect(functionInfo.agentType).to.be.equal(AgentType.TYPE);
      expect(functionInfo.policyCode).to.be.equal(16);
    ...
    it("Should enable activity of memberRemoveFunction success", async () => {
      ...
      // when
      await expect(
        functionManagerDelegateProxy.connect(livelyAdmin).functionUpdateActivityStatus(memberSignature, requests)
      )
        .to.emit(functionManagerDelegateProxy, "FunctionActivityUpdated")
        .withArgs(livelyAdminWallet.address, memberRemoveFunctionId, ActivityStatus.ENABLED);

      // then
      expect(
        await functionManagerDelegateProxy.functionCheckSelector(
          memberManagerProxy.address,
          memberIface.getSighash("memberRemove")
        )
      ).to.be.true;
    ...
    it("Should update admin of memberRegisterFunction when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        functionManagerDelegateProxy.connect(livelyAdmin).functionUpdateAdmin(memberSignature, requests)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should update agent of memberRegisterFunction when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        functionManagerDelegateProxy.connect(livelyAdmin).functionUpdateAgent(memberSignature, requests)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should update policyCode of memberRegisterFunction when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        functionManagerDelegateProxy.connect(livelyAdmin).functionUpdatePolicyCode(memberSignature, requests)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should update alterability of memberRegisterFunction success", async () => {
      ...
      // when
      await expect(
        functionManagerDelegateProxy.connect(systemAdmin).functionUpdateAlterabilityStatus(memberSignature, requests)
      )
        .to.emit(functionManagerDelegateProxy, "FunctionAlterabilityUpdated")
    ...
    it("Should update agent of memberRegisterFunction to aclTypeTest success", async () => {
      ...
      // then
      expect(await functionManagerDelegateProxy.functionCheckAgent(memberRegisterFunctionId, userWallet2.address)).to.be
        .true;
      expect(await functionManagerDelegateProxy.functionCheckAgent(memberRegisterFunctionId, userWallet3.address)).to.be
        .false;
      expect(await functionManagerDelegateProxy.functionCheckAgent(memberRegisterFunctionId, livelyAdminWallet.address))
        .to.be.false;
    });
    ...
    it("Should aclRoleTest3 register user3 with Illegal role scope failed", async () => {
      ...
      // when
      await expect(
        memberManagerDelegateProxy.connect(user2).memberRegister(memberSignature, memberRegisterRequests)
    ...
    it("Should update agent of memberRegisterFunction to aclRoleTest success", async () => {
      ...
      // then
      expect(await functionManagerDelegateProxy.functionCheckAgent(memberRegisterFunctionId, userWallet1.address)).to.be
        .true;
      expect(await functionManagerDelegateProxy.functionCheckAgent(memberRegisterFunctionId, userWallet2.address)).to.be
        .false;
      expect(await functionManagerDelegateProxy.functionCheckAgent(memberRegisterFunctionId, userWallet3.address)).to.be
    ...
    it("Should update admin of memberRegisterFunction success", async () => {
      ...
      // when
      await expect(
        functionManagerDelegateProxy.connect(livelyAdmin).functionUpdateAdmin(memberSignature, updateAdminRequests)
      )
        .to.emit(functionManagerDelegateProxy, "FunctionAdminUpdated")
        .withArgs(livelyAdminWallet.address, memberRegisterFunctionId, aclRoleTestId);

      // then
      expect(await functionManagerDelegateProxy.functionCheckAdmin(memberRegisterFunctionId, userWallet1.address)).to.be
        .true;
    });
    ...
    it("Should update policyCode of memberRegisterFunction to RLOCK failed", async () => {
      ...
      // when
      await expect(
        functionManagerDelegateProxy.connect(user1).functionUpdatePolicyCode(memberSignature, requests)
    ...
    it("Should remove hard remove test context success", async() => {
      ...
      expect(contextInfo.adminId).to.be.equal(ethers.constants.HashZero);
      expect(contextInfo.adminType).to.be.equal(AgentType.NONE);
      expect(contextInfo.contractId).to.be.equal(ethers.constants.AddressZero);
      expect(contextInfo.realmId).to.be.equal(ethers.constants.HashZero);
      expect(contextInfo.functionLimit).to.be.equal(0);
      expect(contextInfo.functionCount).to.be.equal(0);
      expect(contextInfo.acstat).to.be.equal(ActivityStatus.NONE);
      expect(contextInfo.alstat).to.be.equal(AlterabilityStatus.NONE);
      expect(contextInfo.referredByAgent).to.be.equal(0);
    });
    ...
    it("Should update admin of memberContext when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        contextManagerDelegateProxy.connect(livelyAdmin).contextUpdateAdmin(memberSignature, requests)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should update functionLimit of memberContext when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        contextManagerDelegateProxy.connect(livelyAdmin).contextUpdateFunctionLimit(memberSignature, requests)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should update alterability of memberContext success", async () => {
      ...
      // when
      await expect(
        contextManagerDelegateProxy.connect(systemAdmin).contextUpdateAlterabilityStatus(memberSignature, requests)
      )
        .to.emit(contextManagerDelegateProxy, "ContextAlterabilityUpdated")
    ...
    it("Should enable activity of memberContext success", async () => {
      ...
      // then
      expect(
        await contextManagerDelegateProxy.contextHasSelector(
          memberManagerProxy.address,
          memberIface.getSighash("memberRegister")
        )
      ).to.be.true;
    ...
    it("Should update admin of memberContext success", async () => {
      ...
      // when
      await expect(
        contextManagerDelegateProxy.connect(livelyAdmin).contextUpdateAdmin(memberSignature, updateAdminRequests)
      )
    ...
    it("Should hard remove aclDomainTest success", async () => {
      // given
      aclDomainTestId = ethers.utils.keccak256(ethers.utils.toUtf8Bytes(ACL_DOMAIN_TEST_NAME));
      const expiredAt = BigNumber.from(Date.now() + 10000);
      const signature = generateMemberSignatureManually(
      ...
      await expect(domainManagerDelegateProxy.connect(systemAdmin).domainRemove(memberSignature, [aclDomainTestId]))
        ...
        .withArgs(livelyAdminWallet.address, aclDomainTestId, false);
    ...
    it("Should update admin of aclDomainTest when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        domainManagerDelegateProxy.connect(livelyAdmin).domainUpdateAdmin(memberSignature, requests)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should update roleLimit of aclDomainTest when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        domainManagerDelegateProxy.connect(livelyAdmin).domainUpdateRealmLimit(memberSignature, requests)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should update admin of aclDomainTest success", async () => {
      ...
      // when
      await expect(
        domainManagerDelegateProxy.connect(livelyAdmin).domainUpdateAdmin(memberSignature, updateAdminRequests)
      )
    ...
    it("Should register aclRealmTest to aclDomainTestId success", async () => {
      ...
      expect(await realmManagerDelegateProxy.realmHasFunction(LIVELY_VERSE_ACL_REALM_ID, memberRegisterFunctionId)).to
        .be.true;
      expect(
        await realmManagerDelegateProxy.realmHasContext(
          LIVELY_VERSE_ACL_REALM_ID,
          ethers.utils.keccak256(memberManagerProxy.address)
        )
      ).to.be.true;
    ...
    it("Should hard remove aclRealmTest success", async () => {
      ...
      await expect(realmManagerDelegateProxy.connect(livelyAdmin).realmRemove(emptyMemberSignature, [aclRealmTestId]))
        ...
        .withArgs(livelyAdminWallet.address, aclRealmTestId, false);

      // then
      expect(await realmManagerDelegateProxy.realmCheckId(aclRealmTestId)).to.be.false;
      ...
      // and
      expect(await realmManagerDelegateProxy.realmCheckId(aclRealmTestId)).to.be.false;
      expect(await realmManagerDelegateProxy.realmCheckName(ACL_REALM_TEST_NAME)).to.be.false;
    ...
    it("Should update admin of aclRealmTest when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        realmManagerDelegateProxy.connect(livelyAdmin).realmUpdateAdmin(memberSignature, requests)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should update activity of aclRealmTest when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        realmManagerDelegateProxy.connect(livelyAdmin).realmUpdateActivityStatus(memberSignature, requests)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should update contextLimit of aclRealmTest when alterability disabled failed", async () => {
      ...
      // when
      await expect(
        realmManagerDelegateProxy.connect(livelyAdmin).realmUpdateContextLimit(memberSignature, requests)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should update admin of aclRealmTest success", async () => {
      ...
      // when
      await expect(
        realmManagerDelegateProxy.connect(livelyAdmin).realmUpdateAdmin(memberSignature, updateAdminRequests)
      )
    ...
    it("Should disable alterability of universe scope success", async () => {
      ...
      await expect(
        universeManagerDelegateProxy
          .connect(systemAdmin)
          .universeUpdateAlterabilityStatus(memberSignature, AlterabilityStatus.DISABLED)
      )
    ...
    it("Should update admin of universe scope when alterability disabled failed", async () => {
      ...
      await expect(
        universeManagerDelegateProxy.connect(livelyAdmin).universeUpdateAdmin(memberSignature, aclRoleTestId)
      ).revertedWith("Illegal Updatable");
    });
    ...
    it("Should update activity of universe scope when alterability disabled failed", async () => {
      ...
      await expect(
        universeManagerDelegateProxy
          .connect(livelyAdmin)
          .universeUpdateActivityStatus(memberSignature, ActivityStatus.DISABLED)
    ...
    it("Should update alterability of universe scope success", async () => {
      ...
      await expect(
        universeManagerDelegateProxy
          .connect(systemAdmin)
          .universeUpdateAlterabilityStatus(memberSignature, AlterabilityStatus.UPDATABLE)
      )
        .to.emit(universeManagerDelegateProxy, "UniverseAlterabilityUpdated")
    ...
    it("Should register aclDomainTest2 in Universe failed", async () => {
      ...
      // when
      await expect(
        domainManagerDelegateProxy.connect(livelyAdmin).domainRegister(memberSignature, requests)
    ...
    it("Should enable activity of universe scope success", async () => {
      ...
      await expect(
        universeManagerDelegateProxy
          .connect(livelyAdmin)
          .universeUpdateActivityStatus(memberSignature, ActivityStatus.ENABLED)
      )
        .to.emit(universeManagerDelegateProxy, "UniverseActivityUpdated")
    ...
    it("Should update admin of universe scope to aclRoleUniverseAdminTestId success", async () => {
      ...
      await expect(
        universeManagerDelegateProxy
          .connect(livelyAdmin)
          .universeUpdateAdmin(memberSignature, aclRoleUniverseAdminTestId)
      )
    ...
    it("Should update admin of universe scope to LIVELY_MATER_ADMIN success", async () => {
      ...
      await expect(
        universeManagerDelegateProxy
          .connect(user3)
          .universeUpdateAdmin(memberSignature, LIVELY_VERSE_LIVELY_MASTER_ADMIN_ROLE_ID)
      )
        .to.emit(universeManagerDelegateProxy, "UniverseAdminUpdated")
  ...
  describe("Upgrade ACL proxy tests", function () {
    ...
    it("Should enable Upgrade Status of domain proxy by anyone failed", async () => {
      // when and then
      await expect(
        domainManagerProxy.connect(user1).setUpdatabilityStatus(ProxyUpdatabilityStatus.ENABLED)
      ).to.revertedWith("Forbidden");
    });
    ...
    it("Should enable Upgrade Status of domain proxy by livelyAdmin success", async () => {
      ...
      await expect(domainManagerProxy.connect(livelyAdmin).setUpdatabilityStatus(ProxyUpdatabilityStatus.ENABLED))
        ...
        .withArgs(livelyAdminWallet.address, domainManagerProxy.address, ProxyUpdatabilityStatus.ENABLED);
    });
    ...
    it("Should call acl facet upgrade by systemAdmin success", async () => {
      ...
      const domainFunctionUpgradeRequests: IACLManager.FacetSelectorUpgradeRequestStruct[] = [
        ...
        },
        {
          ...
          selectors: [domainTestIface.getSighash("domainRegister2")],
        },
      ];
      ...
      const aclFunctionUpgradeRequests: IACLManager.FacetSelectorUpgradeRequestStruct[] = [
        ...
        {
          action: ActionType.ADD,
          selectors: [aclTestIface.getSighash("reInitialize")],
        },
      ];
      ...
      // and
      const facetDomainInfo: IACLManager.FacetInfoStruct = await aclManagerProxy.aclGetFacetInfo(
        domainManagerProxy.address
      );
      ...
      expect(await aclManagerProxy.aclGetFacet(aclTestIface.getSighash("reInitialize"))).to.be.equal(
        aclManagerProxy.address
      );
    });
    ...
    it("Should ACLManager proxy upgradeTo aclManagerSubjectTest by systemAdmin success", async () => {
      ...
      // when and then
      await expect(aclManagerProxy.connect(systemAdmin).upgradeTo(aclManagerSubjectTest.address, data, false))
        .to.emit(aclManagerProxy, "ProxyUpgraded")
    ...
    it("Should delete of domain old functions in domainContext by livelyAdmin success", async () => {
      ...
      await expect(
        ...
        .to.emit(functionManagerDelegateProxy, "FunctionRemoved")
        .withArgs(livelyAdminWallet.address, domainMoveRealmFunctionId, false)
        .to.emit(functionManagerDelegateProxy, "FunctionRemoved")
        .withArgs(livelyAdminWallet.address, domainRemoveFunctionId, false);
    ...
    it("Should register new domainRegister2 in domainContext by livelyAdmin success", async () => {
      ...
      // and
      expect(await domainManagerDelegateProxyTest.domainHasContext(LIVELY_VERSE_ACL_DOMAIN_ID, domainContextId)).to.be
        .true;
      expect(
        await domainManagerDelegateProxyTest.domainHasFunction(LIVELY_VERSE_ACL_DOMAIN_ID, domainRegisterFunctionId)
      ).to.be.true;
      expect(await domainManagerDelegateProxyTest.domainHasRealm(LIVELY_VERSE_ACL_DOMAIN_ID, LIVELY_VERSE_ACL_REALM_ID))
        .to.be.true;
    });
    ...
    it("Should call domainRegister2 by livelyAdmin success", async () => {
      ...
      expect(await domainManagerDelegateProxyTest.domainCheckAdmin(aclDomainTest2Id, livelyAdminWallet.address)).to.be
        .true;
    });
  });
