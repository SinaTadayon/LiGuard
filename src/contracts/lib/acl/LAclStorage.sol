...
library LAclStorage {
  ...
  function groupReadSlot(AclStorage.DataCollection storage data, bytes32 agentId) internal view returns (IAclCommons.GroupEntity storage ge) {
    IAclCommons.BaseAgent storage ba = data.agents[agentId];
    if(ba.atype == IAclCommons.AgentType.GROUP) {
      assembly {
        let ptr := mload(0x40)
        mstore(add(ptr, 0x00), agentId)
        mstore(add(ptr, 0x20), add(data.slot, 1))
      ...
      revert("GroupId Not Found");
    } else {
      revert("Invalid GroupId Agent Slot");
    } 
  }

  function groupTryReadSlot(AclStorage.DataCollection storage data, bytes32 agentId) internal view returns (IAclCommons.GroupEntity storage ge, bool result) {
    IAclCommons.BaseAgent storage ba = data.agents[agentId];
    if(ba.atype == IAclCommons.AgentType.GROUP) {
      assembly {
        let ptr := mload(0x40)
      ...
      assembly {
        ge.slot := 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff
      }
      result = false;
    }  
  }

  function groupWriteSlot(AclStorage.DataCollection storage data, bytes32 agentId) internal view returns (IAclCommons.GroupEntity storage ge) {
    IAclCommons.BaseAgent storage ba = data.agents[agentId];
    if(ba.atype == IAclCommons.AgentType.NONE || ba.atype == IAclCommons.AgentType.GROUP) {
      assembly {
        let ptr := mload(0x40)
        mstore(add(ptr, 0x00), agentId)
        mstore(add(ptr, 0x20), add(data.slot, 1))
  ...
  function groupTryWriteSlot(AclStorage.DataCollection storage data, bytes32 agentId) internal view returns (IAclCommons.GroupEntity storage ge, bool result) {
    IAclCommons.BaseAgent storage ba = data.agents[agentId];
    if(ba.atype == IAclCommons.AgentType.NONE || ba.atype == IAclCommons.AgentType.GROUP) {
      assembly {
        let ptr := mload(0x40)
