  ...
  function profileDomainRegister(ProfileDomainRegisterRequest[] calldata requests) external returns (bool) {
    for(uint i = 0; i < requests.length; i++) {
      ...
      for(uint j = 0; j < requests[i].domains.length; j++) {
        ...
        IProfileACL.ProfileAccessAdminStatus status = _doCheckAdminAccess(profileEntity, livelyGlobalEntity.bs.adminId, senderId, functionId);
  ...
  function profileDomainUpdateActivityStatus(ProfileUpdateActivityRequest[] calldata requests) external returns (bool) {
    for(uint i = 0; i < requests.length; i++) {
      ...
      for(uint j = 0; j < requests[i].data.length; j++) {
        ...
        IProfileACL.ProfileAccessAdminStatus status = _doCheckAdminAccess(profileEntity, domainEntity.bs.adminId, senderId, functionId);
        if(status != IProfileACL.ProfileAccessAdminStatus.PERMITTED) LACLUtils.generateProfileAdminAccessError(status);
