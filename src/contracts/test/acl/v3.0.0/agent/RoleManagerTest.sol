...
import "./IRoleManagementTest.sol";
 ...
 *
 */
contract RoleManagerTest is ACLStorageTest, BaseUUPSProxy, IRoleManagementTest {
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
    return interfaceId == type(IRoleManagementTest).interfaceId || super.supportsInterface(interfaceId);
  }
    ...
    returns (bool)
  {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IRoleManagementTest.roleRegister.selector
    );
    ...
    for (uint256 i = 0; i < requests.length; i++) {
      (bytes32 newRoleId, bytes32 adminId) = LACLAgentScopeTest.aclRegisterRole(
        _data,
        requests[i],
        functionId,
        senderId,
    ...
    returns (bool)
  {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IRoleManagementTest.roleUpdateAdmin.selector
    );
    ...
    returns (bool)
  {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IRoleManagementTest.roleUpdateScope.selector
    );
    ...
    returns (bool)
  {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IRoleManagementTest.roleUpdateActivityStatus.selector
    );
    for (uint256 i = 0; i < requests.length; i++) {
      RoleEntity storage roleEntity = _doGetEntityAndCheckAdminAccess(
        requests[i].id,
  ...
  function roleUpdateAlterabilityStatus(
    ...
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IRoleManagementTest.roleUpdateAlterabilityStatus.selector
    );
    for (uint256 i = 0; i < requests.length; i++) {
      RoleEntity storage roleEntity = _data.roleReadSlot(requests[i].id);

      IACLTest.AdminAccessStatus status = _doCheckAdminAccess(roleEntity.ba.adminId, senderId, functionId);
    ...
    returns (bool)
  {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IRoleManagementTest.roleUpdateMemberLimit.selector
    );
    for (uint256 i = 0; i < requests.length; i++) {
      RoleEntity storage roleEntity = _doGetEntityAndCheckAdminAccess(
        requests[i].roleId,
    ...
    returns (bool)
  {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IRoleManagementTest.roleGrantMembers.selector
    );
    for (uint256 i = 0; i < requests.length; i++) {
      RoleEntity storage roleEntity = _doGetEntityAndCheckAdminAccess(
        requests[i].roleId,
      ...
      for (uint256 j = 0; j < requests[i].members.length; j++) {
        ...
        if (memberEntity.types.contains(roleEntity.typeId)) {
          ...
          if (requests[i].roleId == LACLAgentScopeTest.LIVELY_VERSE_LIVELY_MASTER_ADMIN_ROLE_ID) {
    ...
    returns (bool)
  {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IRoleManagementTest.roleRevokeMembers.selector
    );
    for (uint256 i = 0; i < requests.length; i++) {
      RoleEntity storage roleEntity = _doGetEntityAndCheckAdminAccess(
        requests[i].roleId,
  ...
  function roleRemove(MemberSignature calldata memberSign, bytes32[] calldata roles) external returns (bool) {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IRoleManagementTest.roleRemove.selector
    );
  ...
  function roleHasAccount(bytes32 roleId, address account) external view returns (bool) {
    return _doRoleHasMember(roleId, LACLUtilsTest.accountGenerateId(account));
  }
    ...
    )
  {
    ...
    if (status != IACLTest.AuthorizationStatus.PERMITTED) {
      if (
        status == IACLTest.AuthorizationStatus.ROLE_ACTIVITY_FORBIDDEN &&
        selector == IRoleManagementTest.roleUpdateActivityStatus.selector
      ) {
  ...
  function _getRoleAdmin(
    ...
    return LACLAgentScopeTest.getRoleAdmin(_data, requestScopeType, requestScopeAdmin, scopeId, adminId);
  }

  function _checkUpdateRequestScope(bytes32 requestScopeId, bytes32 typeScopeId) internal returns (ScopeType) {
    return LACLAgentScopeTest.roleCheckRequestScope(_data, requestScopeId, typeScopeId);
  }
  ...
  function _doGetEntityAndCheckAdminAccess(
    ...
    if (status != IACLTest.AdminAccessStatus.PERMITTED) {
      if (
        status == IACLTest.AdminAccessStatus.ROLE_ACTIVITY_FORBIDDEN &&
        selector == IRoleManagementTest.roleUpdateActivityStatus.selector
      ) {
        return roleEntity;
      }
