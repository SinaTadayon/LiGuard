 ...
 *
 */
contract MemberManagerTest is ACLStorageTest, BaseUUPSProxy, IMemberManagementTest {
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
    return interfaceId == type(IMemberManagementTest).interfaceId || super.supportsInterface(interfaceId);
  }
    ...
    returns (bool)
  {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IMemberManagementTest.memberRegister.selector
    );
    ...
    returns (bool)
  {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IMemberManagementTest.memberUpdateActivityStatus.selector
    );

    for (uint256 i = 0; i < requests.length; i++) {
      MemberEntity storage memberEntity = _doGetEntityAndCheckAdminAccess(requests[i].id, senderId, functionId);
      require(
        !_doRoleHasMember(LACLAgentScopeTest.LIVELY_VERSE_LIVELY_MASTER_ADMIN_ROLE_ID, requests[i].id),
        "Illegal Member"
      );
  ...
  function memberUpdateAlterabilityStatus(
    ...
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IMemberManagementTest.memberUpdateAlterabilityStatus.selector
    );

    for (uint256 i = 0; i < requests.length; i++) {
      MemberEntity storage memberEntity = _data.memberReadSlot(requests[i].id);
      IACLTest.AdminAccessStatus adminAccessStatus = _doCheckAdminAccess(memberEntity.ba.adminId, senderId, functionId);
      if (adminAccessStatus != IACLTest.AdminAccessStatus.PERMITTED) LACLUtilsTest.generateAdminAccessError(adminAccessStatus);
    ...
    returns (bool)
  {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IMemberManagementTest.memberUpdateAdmin.selector
    );
  ...
  function memberUpdateGeneralLimit(
    ...
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IMemberManagementTest.memberUpdateGeneralLimit.selector
    );
  ...
  function memberRemove(MemberSignature calldata memberSign, bytes32[] calldata members) external returns (bool) {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IMemberManagementTest.memberRemove.selector
    );

    for (uint256 i = 0; i < members.length; i++) {
      MemberEntity storage memberEntity = _data.memberReadSlot(members[i]);
      IACLTest.AdminAccessStatus status = _doCheckAdminAccess(memberEntity.ba.adminId, senderId, functionId);
      ...
      for (uint256 j = 0; j < memberEntity.types.length() && j < 16; j++) {
        ...
        if (roleId == LACLAgentScopeTest.LIVELY_VERSE_LIVELY_MASTER_ADMIN_ROLE_ID) {
  ...
  function _doMemberRegister(
    ...
    bytes32 newMemberId = LACLUtilsTest.accountGenerateId(request.account);
    ...
    // check access
    IACLTest.AdminAccessStatus adminAccessStatus = _doCheckAdminAccess(roleEntity.ba.adminId, senderId, functionId);
    if (adminAccessStatus != IACLTest.AdminAccessStatus.PERMITTED) LACLUtilsTest.generateAdminAccessError(adminAccessStatus);
  ...
  function _doCheckMemberRegisterLimits(MemberEntity storage memberEntity, GeneralLimit calldata limits) internal view {
    LACLAgentScopeTest.checkMemberRegisterLimits(memberEntity, limits);
  }
