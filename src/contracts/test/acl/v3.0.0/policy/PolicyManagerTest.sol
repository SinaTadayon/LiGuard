...
import "./IPolicyManagementTest.sol";
import "../IACLTest.sol";
 ...
 *
 */
contract PolicyManagerTest is ACLStorageTest, BaseUUPSProxy, IPolicyManagementTest {
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
    return interfaceId == type(IPolicyManagementTest).interfaceId || super.supportsInterface(interfaceId);
  }
  ...
  function policyRegister(MemberSignature calldata memberSign, PolicyRegisterRequest[] calldata requests)
    ...
    returns (bool)
  {
    (, bytes32 senderId, address sender) = _accessPermission(memberSign, IPolicyManagementTest.policyRegister.selector);
    ...
    returns (bool)
  {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IPolicyManagementTest.policyAddRoles.selector
    );
    for (uint256 i = 0; i < requests.length; i++) {
      ...
      for (uint256 j = 0; j < requests[i].roles.length; j++) {
        ...
        if (roleScopeType == policyScopeType) {
          ...
          require(
            IACLGeneralsTest(address(this)).isScopesCompatible(policyEntity.scopeId, roleEntity.scopeId),
            "Illegal Role Scope"
          );
        }
    ...
    returns (bool)
  {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IPolicyManagementTest.policyRemoveRoles.selector
    );
    for (uint256 i = 0; i < requests.length; i++) {
      PolicyEntity storage policyEntity = _doGetPolicyAndCheckAdminAccess(requests[i].policyId, senderId, functionId);
    ...
    returns (bool)
  {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IPolicyManagementTest.policyUpdateCodes.selector
    );
    for (uint256 i = 0; i < requests.length; i++) {
      PolicyEntity storage policyEntity = _doGetPolicyAndCheckAdminAccess(requests[i].policyId, senderId, functionId);
    ...
    returns (bool)
  {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IPolicyManagementTest.policyUpdateAdmin.selector
    );
    for (uint256 i = 0; i < requests.length; i++) {
    ...
    returns (bool)
  {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IPolicyManagementTest.policyUpdateScope.selector
    );
    for (uint256 i = 0; i < requests.length; i++) {
      LACLAgentScopeTest.updatePolicyScope(_data, requests[i], functionId, senderId);
    ...
    returns (bool)
  {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IPolicyManagementTest.policyUpdateActivityStatus.selector
    );
    for (uint256 i = 0; i < requests.length; i++) {
      PolicyEntity storage policyEntity = _doGetPolicyAndCheckAdminAccess(requests[i].id, senderId, functionId);
  ...
  function policyUpdateAlterabilityStatus(
    ...
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IPolicyManagementTest.policyUpdateAlterabilityStatus.selector
    );
    for (uint256 i = 0; i < requests.length; i++) {
      PolicyEntity storage policyEntity = _data.policies[requests[i].id];
      require(policyEntity.adminId != bytes32(0), "Not Found");

      IACLTest.AdminAccessStatus status = _doCheckAdminAccess(policyEntity.adminId, senderId, functionId);
    ...
    returns (bool)
  {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IPolicyManagementTest.policyUpdateRoleLimit.selector
    );
    for (uint256 i = 0; i < requests.length; i++) {
      PolicyEntity storage policyEntity = _doGetPolicyAndCheckAdminAccess(requests[i].policyId, senderId, functionId);
  ...
  function policyRemove(MemberSignature calldata memberSign, bytes32[] calldata policies) external returns (bool) {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IPolicyManagementTest.policyRemove.selector
    );
    for (uint256 i = 0; i < policies.length; i++) {
      IACLCommonsTest.PolicyEntity storage policyEntity = _data.policies[policies[i]];
      require(policyEntity.adminId != bytes32(0), "Not Found");
  ...
  function policyCheckName(string calldata policyName) external view returns (bool) {
    return _data.policies[LACLUtilsTest.generateId(policyName)].adminId != bytes32(0);
  }
  ...
  function _getPolicyAdmin(
    ...
    return LACLAgentScopeTest.getPolicyAdmin(_data, requestScopeType, requestScopeAdmin, scopeId, adminId);
  }
  ...
  function _doGetPolicyAndCheckAdminAccess(
    ...
    return LACLAgentScopeTest.getPolicyAndCheckAdminAccess(_data, policyId, memberId, functionId);
  }
  ...
  function _getCheckUpdateRequestScope(
    ...
    return LACLAgentScopeTest.getCheckUpdateRequestScope(_data, requestScopeId, senderScopeId, senderScopeType);
  }
