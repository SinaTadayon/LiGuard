...
import "./IContextManagementTest.sol";
 ...
 *
 */
contract ContextManagerTest is ACLStorageTest, BaseUUPSProxy, IContextManagementTest {
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
    return interfaceId == type(IContextManagementTest).interfaceId || super.supportsInterface(interfaceId);
  }
    ...
    returns (bool)
  {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IContextManagementTest.contextUpdateActivityStatus.selector
    );
    ...
    for (uint256 i = 0; i < requests.length; i++) {
      ...
      IACLTest.AdminAccessStatus status = _doCheckAdminAccess(contextEntity.bs.adminId, senderId, functionId);
  ...
  function contextUpdateAlterabilityStatus(
    ...
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IContextManagementTest.contextUpdateAlterabilityStatus.selector
    );
    ...
    returns (bool)
  {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IContextManagementTest.contextUpdateAdmin.selector
    );
  ...
  function contextUpdateFunctionLimit(
    ...
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IContextManagementTest.contextUpdateFunctionLimit.selector
    );
  ...
  function contextRemove(MemberSignature calldata memberSign, bytes32[] calldata contexts) external returns (bool) {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IContextManagementTest.contextRemove.selector
    );
  ...
  function contextCheckAccount(address contractId) external view returns (bool) {
    return _data.scopes[LACLUtilsTest.accountGenerateId(contractId)].stype == ScopeType.CONTEXT;
  }
    ...
    )
  {
    ...
    if (status != IACLTest.AuthorizationStatus.PERMITTED) {
      if (
        status == IACLTest.AuthorizationStatus.CONTEXT_ACTIVITY_FORBIDDEN &&
        IContextManagementTest.contextUpdateActivityStatus.selector == selector
      ) {
  ...
  function _doRegisterContext(
    ...
    bytes32 functionId = LACLUtilsTest.functionGenerateId(
      _data.selectors[IContextManagementTest.contextRegister.selector],
