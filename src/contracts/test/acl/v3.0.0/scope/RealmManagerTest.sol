...
import "./IRealmManagementTest.sol";
import "../IACLTest.sol";
 ...
 *
 */
contract RealmManagerTest is ACLStorageTest, BaseUUPSProxy, IRealmManagementTest {
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
    return interfaceId == type(IRealmManagementTest).interfaceId || super.supportsInterface(interfaceId);
  }
    ...
    returns (bool)
  {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IRealmManagementTest.realmRegister.selector
    );
    ...
    for (uint256 i = 0; i < requests.length; i++) {
      bytes32 newRealmId = LACLUtilsTest.generateId(requests[i].name);
    ...
    returns (bool)
  {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IRealmManagementTest.realmUpdateAdmin.selector
    );
    ...
    for (uint256 i = 0; i < requests.length; i++) {
      ...
      if (requests[i].adminId != bytes32(0)) {
        ...
        if (ScopeType.REALM == requestAdminScopeType) {
          ...
          require(
            IACLGeneralsTest(address(this)).isScopesCompatible(requestAdminScopeId, requests[i].id),
            "Illegal Admin Scope"
          );
        }
    ...
    returns (bool)
  {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IRealmManagementTest.realmMoveContext.selector
    );
    ...
    for (uint256 i = 0; i < requests.length; i++) {
      ...
      require(
        IACLGeneralsTest(address(this)).isScopesCompatible(contextAdminScopeId, requests[i].targetRealmId),
        "Illegal Admin Scope"
      );
    ...
    returns (bool)
  {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IRealmManagementTest.realmUpdateActivityStatus.selector
    );
    ...
    for (uint256 i = 0; i < requests.length; i++) {
      ...
      IACLTest.AdminAccessStatus status = _doCheckAdminAccess(realmEntity.bs.adminId, senderId, functionId);
      if (status != IACLTest.AdminAccessStatus.PERMITTED) LACLUtilsTest.generateAdminAccessError(status);
  ...
  function realmUpdateAlterabilityStatus(
    ...
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IRealmManagementTest.realmUpdateAlterabilityStatus.selector
    );
  ...
  function realmUpdateContextLimit(
    ...
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IRealmManagementTest.realmUpdateContextLimit.selector
    );
  ...
  function realmRemove(MemberSignature calldata memberSign, bytes32[] calldata realms) external returns (bool) {
    (bytes32 functionId, bytes32 senderId, address sender) = _accessPermission(
      memberSign,
      IRealmManagementTest.realmRemove.selector
    );
  ...
  function realmCheckName(string calldata realmName) external view returns (bool) {
    return _data.scopes[LACLUtilsTest.generateId(realmName)].stype == ScopeType.REALM;
  }
    ...
    )
  {
    ...
    if (status != IACLTest.AuthorizationStatus.PERMITTED) {
      if (
        status == IACLTest.AuthorizationStatus.REALM_ACTIVITY_FORBIDDEN &&
        selector == IRealmManagementTest.realmUpdateActivityStatus.selector
      ) {
  ...
  function _getRealmAdmin(
    ...
    return LACLCommonsTest.realmGetAdmin(_data, requestScopeAdmin, domainId, adminId);
  }
