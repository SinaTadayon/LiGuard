...
import "./IProfileContextManagementTest.sol";
 ...
 *
 */
contract ProfileContextManagerTest is ACLStorageTest, BaseUUPSProxy, IProfileContextManagementTest {
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
    return interfaceId == type(IProfileContextManagementTest).interfaceId || super.supportsInterface(interfaceId);
  }
  ...
  function profileContextRegister(
    ...
    for (uint256 i = 0; i < requests.length; i++) {
      ...
      bytes32 newContextId = LProfileCommonsTest.profileRegisterContext(_data, requests[i], profileId, contractId, signer);
  ...
  function profileContextUpdateActivityStatus(
    ...
    (
      ...
      bytes32 senderId,
      address sender
    ) = _accessPermission(memberSign, IProfileContextManagementTest.profileContextUpdateActivityStatus.selector);
    for (uint256 i = 0; i < requests.length; i++) {
  ...
  function profileContextUpdateAlterabilityStatus(
    ...
    (
      ...
      bytes32 senderId,
      address sender
    ) = _accessPermission(memberSign, IProfileContextManagementTest.profileContextUpdateAlterabilityStatus.selector);
    for (uint256 i = 0; i < requests.length; i++) {
      ContextEntity storage contextEntity = profileEntity.profileContextReadSlot(requests[i].entityId);
  ...
  function profileContextUpdateAdmin(
    ...
    (
      ...
      bytes32 senderId,
      address sender
    ) = _accessPermission(memberSign, IProfileContextManagementTest.profileContextUpdateAdmin.selector);
    for (uint256 i = 0; i < requests.length; i++) {
  ...
  function profileContextUpdateFunctionLimit(
    ...
    (
      ...
      bytes32 senderId,
      address sender
    ) = _accessPermission(memberSign, IProfileContextManagementTest.profileContextUpdateFunctionLimit.selector);
    for (uint256 i = 0; i < requests.length; i++) {
      ContextEntity storage contextEntity = _doGetEntityAndCheckAdminAccess(
        profileEntity,
        functionEntity,
    ...
    returns (bool)
  {
    (
      ...
      bytes32 senderId,
      address sender
    ) = _accessPermission(memberSign, IProfileContextManagementTest.profileContextRemove.selector);
  ...
  function profileContextCheckAccount(bytes32 profileId, address contractId) external view returns (bool) {
    return _data.profiles[profileId].scopes[LACLUtilsTest.accountGenerateId(contractId)].stype == ScopeType.CONTEXT;
  }
  ...
  {
    return LProfileCommonsTest.profileAgentGetScopeInfo(profileEntity, agentId);
  }
