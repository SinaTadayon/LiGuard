...
import "./IProfileRealmManagementTest.sol";
 ...
 *
 */
contract ProfileRealmManagerTest is ACLStorageTest, BaseUUPSProxy, IProfileRealmManagementTest {
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
    return interfaceId == type(IProfileRealmManagementTest).interfaceId || super.supportsInterface(interfaceId);
  }
  ...
  function profileRealmRegister(
    ...
    (
      ...
      bytes32 senderId,
      address sender
    ) = _accessPermission(memberSign, IProfileRealmManagementTest.profileRealmRegister.selector);
    LProfileCommonsTest.profileCheckMemberForRealmRegister(profileEntity, uint16(requests.length), senderId);
  ...
  function profileRealmUpdateAdmin(
    ...
    (
      ...
      bytes32 senderId,
      address sender
    ) = _accessPermission(memberSign, IProfileRealmManagementTest.profileRealmUpdateAdmin.selector);
    for (uint256 i = 0; i < requests.length; i++) {
  ...
  function profileRealmMoveContext(
    ...
    (
      ...
      bytes32 senderId,
      address sender
    ) = _accessPermission(memberSign, IProfileRealmManagementTest.profileRealmMoveContext.selector);
    for (uint256 i = 0; i < requests.length; i++) {
  ...
  function profileRealmUpdateActivityStatus(
    ...
    (
      ...
      bytes32 senderId,
      address sender
    ) = _accessPermission(memberSign, IProfileRealmManagementTest.profileRealmUpdateActivityStatus.selector);
    for (uint256 i = 0; i < requests.length; i++) {
  ...
  function profileRealmUpdateAlterabilityStatus(
    ...
    (
      ...
      bytes32 senderId,
      address sender
    ) = _accessPermission(memberSign, IProfileRealmManagementTest.profileRealmUpdateAlterabilityStatus.selector);
    for (uint256 i = 0; i < requests.length; i++) {
      RealmEntity storage realmEntity = profileEntity.profileRealmReadSlot(requests[i].entityId);
  ...
  function profileRealmUpdateContextLimit(
    ...
    (
      ...
      bytes32 senderId,
      address sender
    ) = _accessPermission(memberSign, IProfileRealmManagementTest.profileRealmUpdateContextLimit.selector);
    for (uint256 i = 0; i < requests.length; i++) {
      RealmEntity storage realmEntity = _doGetEntityAndCheckAdminAccess(
        profileEntity,
        functionEntity,
    ...
    returns (bool)
  {
    (
      ...
      bytes32 senderId,
      address sender
    ) = _accessPermission(memberSign, IProfileRealmManagementTest.profileRealmRemove.selector);
  ...
  function profileRealmCheckName(bytes32 profileId, string calldata realmName) external view returns (bool) {
    return _data.profiles[profileId].scopes[LACLUtilsTest.generateId(realmName)].stype == ScopeType.REALM;
  }
  ...
  function _doProfileRealmRegister(
    ...
    bytes32 newRealmId = LProfileCommonsTest.profileRealmRegister(
      request,
      profileEntity,
      functionEntity,
      senderId,
