...
import "./IProfileFunctionManagementTest.sol";
 ...
 *
 */
contract ProfileFunctionManagerTest is ACLStorageTest, BaseUUPSProxy, IProfileFunctionManagementTest {
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
    return interfaceId == type(IProfileFunctionManagementTest).interfaceId || super.supportsInterface(interfaceId);
  }
  ...
  function profileFunctionRegister(
    ...
    for (uint256 i = 0; i < requests.length; i++) {
      ...
      {
        address functionFacetId = _data.selectors[IProfileFunctionManagementTest.profileFunctionRegister.selector];
        bytes32 functionId = LACLUtilsTest.functionGenerateId(
          functionFacetId,
          IProfileFunctionManagementTest.profileFunctionRegister.selector
        );
        ProfileAccessControlTest(payable(address(this))).profileAclHasMemberAccess(profileId, functionId, signerId);
      }

      LProfileCommonsTest.profileCheckMemberForFunctionRegister(
        profileEntity,
  ...
  function profileFunctionUpdateAdmin(
    ...
    (
      ...
      bytes32 senderId,
      address sender
    ) = _accessPermission(memberSign, IProfileFunctionManagementTest.profileFunctionUpdateAdmin.selector);
    for (uint256 i = 0; i < requests.length; i++) {
  ...
  function profileFunctionUpdateAgent(
    ...
    (
      ...
      bytes32 senderId,
      address sender
    ) = _accessPermission(memberSign, IProfileFunctionManagementTest.profileFunctionUpdateAgent.selector);
    for (uint256 i = 0; i < requests.length; i++) {
      FunctionEntity storage functionData = _doGetEntityAndCheckAdminAccess(
        profileEntity,
        functionEntity,
  ...
  function profileFunctionUpdateActivityStatus(
    ...
    (
      ...
      bytes32 senderId,
      address sender
    ) = _accessPermission(memberSign, IProfileFunctionManagementTest.profileFunctionUpdateActivityStatus.selector);
    for (uint256 i = 0; i < requests.length; i++) {
  ...
  function profileFunctionUpdateAlterabilityStatus(
    ...
    (
      ...
      bytes32 senderId,
      address sender
    ) = _accessPermission(memberSign, IProfileFunctionManagementTest.profileFunctionUpdateAlterabilityStatus.selector);
    for (uint256 i = 0; i < requests.length; i++) {
      FunctionEntity storage functionData = profileEntity.profileFunctionReadSlot(requests[i].entityId);
  ...
  function profileFunctionUpdatePolicyCode(
    ...
    (
      ...
      bytes32 senderId,
      address sender
    ) = _accessPermission(memberSign, IProfileFunctionManagementTest.profileFunctionUpdatePolicyCode.selector);
    for (uint256 i = 0; i < requests.length; i++) {
    ...
    returns (bool)
  {
    (
      ...
      bytes32 senderId,
      address sender
    ) = _accessPermission(memberSign, IProfileFunctionManagementTest.profileFunctionRemove.selector);
  ...
  function profileFunctionCheckSelector(
    ...
    return
      _data.profiles[profileId].scopes[LACLUtilsTest.functionGenerateId(contractId, selector)].stype == ScopeType.FUNCTION;
  }
  ...
  function _doGetAndCheckFunctionAdmin(
    ...
    return
      LProfileCommonsTest.profileGetAndCheckFunctionAdmin(profileEntity, contextAdminId, contextId, adminId, profileId);
  }
  ...
  function _doCheckSystemScope(
    ...
    IACLCommonsTest.TypeEntity storage systemType = profileEntity.profileTypeReadSlot(
    ...
    if (profileEntity.scopes[memberSystemRole.scopeId].stype < IACLCommonsTest.ScopeType.CONTEXT) return false;
  ...
  function _doProfileFunctionRegister(
    ...
    for (uint256 j = 0; j < request.functions.length; j++) {
      bytes32 functionId = LProfileCommonsTest.profileFunctionRegistration(
        profileEntity,
        request.functions[j],
        profileId,
        contextId
