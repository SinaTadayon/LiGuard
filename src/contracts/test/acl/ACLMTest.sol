 ...
 *
 */
contract ACLMTest is ACLStorage, BaseUUPSProxy, IACLManager {
  ...
  function supportsInterface(bytes4 interfaceId) public view override returns (bool) {
    return
      ...
      interfaceId == type(IDMTest).interfaceId ||
  ...
  function _doAclRegisterFacet(FacetRegisterRequest[] calldata requests) internal returns (bool) {
    for (uint256 i = 0; i < requests.length; i++) {
      LACLMTest.aclRegisterFacet(_data, requests[i]);
