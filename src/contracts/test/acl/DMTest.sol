 ...
 *
 */
contract DMTest is ACLStorage, BaseUUPSProxy, IDMTest {
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
    return interfaceId == type(IDMTest).interfaceId || super.supportsInterface(interfaceId);
  }
  ...
  function domainRegister2(DomainRegisterRequest[] calldata requests) external returns (bool) {
    bytes32 functionId = _accessPermission(IDMTest.domainRegister2.selector);
  ...
  function domainUpdateActivityStatus(UpdateActivityRequest[] calldata requests) external returns (bool) {
    bytes32 functionId = _accessPermission(IDMTest.domainUpdateActivityStatus.selector);
    bytes32 senderId = LACLUtils.accountGenerateId(msg.sender);
    for (uint256 i = 0; i < requests.length; i++) {
  ...
  function domainUpdateAlterabilityStatus(UpdateAlterabilityRequest[] calldata requests) external returns (bool) {
    bytes32 functionId = _accessPermission(IDMTest.domainUpdateAlterabilityStatus.selector);
    bytes32 senderId = LACLUtils.accountGenerateId(msg.sender);
  ...
  function domainUpdateAdmin(UpdateAdminRequest[] calldata requests) external returns (bool) {
    bytes32 functionId = _accessPermission(IDMTest.domainUpdateAdmin.selector);
    bytes32 senderId = LACLUtils.accountGenerateId(msg.sender);
  ...
  function domainUpdateRealmLimit(DomainUpdateRealmLimitRequest[] calldata requests) external returns (bool) {
    bytes32 functionId = _accessPermission(IDMTest.domainUpdateRealmLimit.selector);
    bytes32 senderId = LACLUtils.accountGenerateId(msg.sender);
